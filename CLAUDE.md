...
- Both paths are permanently supported and must render identically (git/PR data, sort order, notifications). One known exception to "the `notify` hook fires once per real transition": a repeat `Done` event arriving while that session's cleanup is still in flight is skipped along with its hook, on both paths. Measured on the daemon path at 5 hook invocations for 7 transitions with the first effect blocked. Toasts are unaffected - they are per-client and ungated. See the handoff for the numbers and their caveat
